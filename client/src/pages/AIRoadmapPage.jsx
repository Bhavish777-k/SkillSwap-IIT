...
const AIRoadmapPage = () => {
    ...
    const generateRoadmap = async () => {
        try {
            ...
            const normalizedRoadmap = normalizeRoadmap(data, targetRole);
            const rawRoadmap = normalizedRoadmap.aiRawResponse;
            const hasRawContent = typeof rawRoadmap === 'string'
                ? rawRoadmap.trim().length > 0
                : isObject(rawRoadmap) && Object.keys(rawRoadmap).length > 0;
            const hasRoadmapContent = Boolean(
                hasRawContent ||
                normalizedRoadmap.currentSkills.length ||
                normalizedRoadmap.missingSkills.length ||
                Object.keys(normalizedRoadmap.missingByCategory).length
            );

            if (!hasRoadmapContent) {
                setError('The AI service returned no roadmap. Please try again.');
                setResult(null);
                return;
            }

            setResult(normalizedRoadmap);
        } catch (err) {
            console.error('AI Roadmap Error:', err);

            const responseError = err.response?.data;
            const backendMessage = typeof responseError === 'string'
                ? responseError.trim()
                : responseError?.message || responseError?.detail;
