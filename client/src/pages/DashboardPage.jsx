  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900">Welcome back, {dashboard?.user?.name}!</h1>
          ...
          <div className="card">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-primary-600">{dashboard?.stats?.sentRequests || 0}</p>
              </div>
          ...
          <div className="card">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-green-600">{dashboard?.stats?.receivedRequests || 0}</p>
              </div>
              <div className="text-4xl">📥</div>
            </div>
          </div>
          ...
          <div className="card">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-blue-600">{dashboard?.stats?.upcomingSessions || 0}</p>
              </div>
              <div className="text-4xl">📅</div>
            </div>
          </div>
          ...
          <div className="lg:col-span-2 space-y-6">
              ...
              <div className="mb-6">
                ...
                {dashboard?.user?.skillsOffered?.length > 0 ? (
                  ...
                  <p className="text-gray-500 text-sm">
                    No skills added yet. 
                    <Link to="/profile" className="text-primary-600 ml-1">Add skills</Link>
                  </p>
                )}
              </div>

              <div>
                <h3 className="text-sm font-medium text-gray-700 mb-3">Skills You Need</h3>
            ...
            <div className="card">
              ...
              <div className="grid grid-cols-2 gap-4">
                <Link to="/browse" className="btn btn-primary text-center">
                  🔍 Browse Mentors
                </Link>
                <Link to="/requests" className="btn btn-secondary text-center">
                  📨 View Requests
                </Link>
                <Link to="/sessions" className="btn btn-secondary text-center">
                  📅 My Sessions
                </Link>
            ...
            <div className="card">
              ...
              {recommendations.length > 0 ? (
                <div className="space-y-4">
                  {recommendations.map((match) => (
                    ...
                    >
                      <div className="flex items-center gap-3 mb-2">
                        <img
                          src={match.user.avatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(match.user.name)}&size=40&background=random`}
                          alt={match.user.name}
                      ...
                      {match.user.skillsOffered && match.user.skillsOffered.length > 0 && (
                        <div className="flex flex-wrap gap-1">
                          {match.user.skillsOffered.slice(0, 2).map((skill) => (
                            <span key={skill._id} className="text-xs badge badge-primary">
                              {skill.name}
                            </span>
                          ))}
                        </div>
                  ...
                  <Link to="/browse" className="btn btn-outline w-full text-sm">
                    View All Matches
                  </Link>
                </div>
              ) : (
