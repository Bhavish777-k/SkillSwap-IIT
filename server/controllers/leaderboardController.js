...
export const getTeacherLeaderboard = async(req, res, next) => {
    try {
        ...
        const leaderboard = await Session.aggregate([
            ...
            // --------------------------------------------------
            {
                $project: {
                    ...
                    averageRating: {
                        $ifNull: ['$reviewStats.averageRating', 0]
                    },

                    reviewCount: {
                        $ifNull: ['$reviewStats.reviewCount', 0]
                    }
                }
            }
        ]);
        ...
        const maxTeachingMinutes = Math.max(
            ...leaderboard.map(item => item.totalTeachingMinutes),
            1
        );
        ...
        const rankedLeaderboard = leaderboard.map(item => {
            ...
            const timeScore =
                (item.totalTeachingMinutes / maxTeachingMinutes) * 30;
        ...
        rankedLeaderboard.sort(
            (a, b) =>
            b.leaderboardScore - a.leaderboardScore
        );
        ...
        const finalLeaderboard = rankedLeaderboard.map(
            (teacher, index) => ({
                rank: index + 1,
                ...teacher
            })
